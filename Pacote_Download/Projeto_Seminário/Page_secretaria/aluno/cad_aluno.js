...
function carregarEstados() {
    fetch('https://servicodados.ibge.gov.br/api/v1/localidades/estados?orderBy=nome')
        ...
        .then(estados => {
            const estadoSelect = document.getElementById('aluno_estado');
...
function carregarCidades(estadoId) {
    const cidadeSelect = document.getElementById('aluno_cidade'); // 
...
// Adicionar eventos
document.getElementById('aluno_estado').addEventListener('change', function() { // Ajustado para aluno_estado
    const estadoId = this.value;
    if (estadoId) {
        carregarCidades(estadoId);
    } else {
        document.getElementById('aluno_cidade').disabled = true; 
        document.getElementById('aluno_cidade').innerHTML = '<option value="">Selecione uma cidade</option>';
    }
});
